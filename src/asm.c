...
struct asmsect_s* scopeitosect(long scopei) {
	...
	for (int i = 0; i < lexa.sectc; ++i) 
		for (int j = 0; j < lexa.sectv[i].scopeic; ++j)
			if (scopei == lexa.sectv[i].scopeiv[j]) {
				sect = lexa.sectv + i;
				break;}
	return sect;}
...
void asmprog() {
	...
	case VARDEF: {
		...
		if (!sect) {
			...
			*sect = (struct asmsect_s){var->sectiv, var->sectic, 0, 1, 1, 1, 0, 0, 1, 0, 0, 0};
			sect->scopeiv = realloc(sect->scopeiv, sizeof(long) * sect->scopeic);
			sect->scopeiv[0] = lexa.tkni;
		...
		else { // add scope entry for converting scopes to sects
			sect->scopeiv = realloc(sect->scopeiv, sizeof(long) * ++sect->scopeic);
			sect->scopeiv[0] = lexa.tkni;}
